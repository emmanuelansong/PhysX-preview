...
namespace VisualDebugger
{
	...
	void HUDInit()
	{
		...
		hud.AddLine(HELP, "");
		hud.AddLine(HELP, "G - Start the domino show");
		hud.AddLine(HELP, "R - Ruin the scene");
		//add a pause screen
		hud.AddLine(PAUSE, "");
		hud.AddLine(PAUSE, "");
		hud.AddLine(PAUSE, "");
	...
	void UserKeyPress(int key)
	{
		switch (toupper(key))
		{
		//drop ball, begin domino show
		case 'G':
			
			scene->GetSelectedActor()->setActorFlag(PxActorFlag::eDISABLE_GRAVITY, false);
			scene->GetSelectedActor()->addForce(PxVec3(0, -1, 0) * 1);
			break;
		default:
			break;
		}
